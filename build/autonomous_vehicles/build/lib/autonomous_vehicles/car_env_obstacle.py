...
class GazeboLidarMaskEnvObstacle(gym.Env):
    ...
    def __init__(self, H=256, W=256, time_step=0.1, max_steps=500):
        ...
        self.obstacle_pass_reward = 500.0
    ...
    def _get_obs_blocking(self, timeout=1.0, require_new=True, last_lidar_seq=-1, last_mask_seq=-1):
        ...
        while time.time() - t0 < timeout:
            ...
            # cv2.imshow("img1", mask_img)
            # cv2.imshow("img1", depth_u8)
    ...
    def _compute_reward(self):
            ...
            f"progR={progress_reward:.2f} edgeP={edge_penalty:.2f} spiralP={spiral_penalty:.2f} "
            f"total={reward:.2f}"
)
            return float(reward)
