...
import matplotlib
matplotlib.use("Qt5Agg")  # albo "TkAgg" jeśli nie masz Qt
import matplotlib.pyplot as plt
import numpy as np
# =========================
# Ustawienia
...
class FastLiveRewardPlotCallback(BaseCallback):
    """
    Szybki live wykres w osobnym oknie: reward(t) i return(t)=sum(reward).
    Używa blit -> minimalny narzut.
    """
    def __init__(self, env_idx=0, update_every=2, verbose=0):
    ...
    def _on_training_start(self) -> None:
        ...
        self.ax.set_xlabel("step w epizodzie")
        self.ax.set_ylabel("reward / return")
        self.ax.grid(True)

        (self.line_r,) = self.ax.plot([], [], label="reward(t)")
        (self.line_ret,) = self.ax.plot([], [], label="return(t)")
    ...
    def _on_step(self) -> bool:
        rewards = self.locals["rewards"]   # np.array(n_envs,)
        dones   = self.locals["dones"]     # np.array(n_envs,)

        rt = float(rewards[self.env_idx])
        done = bool(dones[self.env_idx])

        self.r.append(rt)
        self.ret.append((self.ret[-1] if self.ret else 0.0) + rt)
        self.t += 1
        ...
        if (self.t % self.update_every) == 0:
            ...
            # update danych
            self.line_r.set_data(x, self.r)
            self.line_ret.set_data(x, self.ret)

            # szybkie dopasowanie osi (bez ciężkiego autoscale co klatkę)
            ymin = min(min(self.r), min(self.ret))
            ymax = max(max(self.r), max(self.ret))
            pad = 0.1 * (ymax - ymin + 1e-6)
            self.ax.set_xlim(0, max(50, self.t))
            self.ax.set_ylim(ymin - pad, ymax + pad)
            self.ax.set_title(f"Episode {self.ep+1} | step={self.t} | return={self.ret[-1]:.2f}")

            # BLIT (szybko)
            self.fig.canvas.restore_region(self.bg)
            self.ax.draw_artist(self.line_r)
            self.ax.draw_artist(self.line_ret)
        ...
        if done:
            ...
            # odśwież tło po resecie (żeby tytuł/osi nie “smużyły”)
            self.fig.canvas.draw()
            self.bg = self.fig.canvas.copy_from_bbox(self.ax.bbox)
...
model = PPO.load("/home/developer/ros2_ws/src/cheakpoints/checkpoint_380000_steps.zip", env=vec_env, device=DEVICE)
...
live_plot_cb = FastLiveRewardPlotCallback(env_idx=0, update_every=2)

callback = CallbackList([
    checkpoint_callback,
    live_plot_cb,
    # (opcjonalnie) wandb_callback — możesz wywalić, jeśli tylko spowalnia
])
